...
import com.htmlhifive.sync.ctrl.SyncDownloadResult;
import com.htmlhifive.sync.ctrl.SyncResultType;
import com.htmlhifive.sync.ctrl.SyncUploadResult;
import com.htmlhifive.sync.ctrl.Synchronizer;
...
public class JsonSyncController {
    ...
    public ResponseEntity<DownloadResponseOnInit> syncInit(
        ...
        try {
            ...
            AbstractSyncResult downloadResult =
                    synchronizer.syncDownload(newStorageId, request.getResources());
     ...
     *
     * @param request
     *            JSON形式の同期リクエストデータ(下り更新用)
     * @return JSON形式の同期レスポンスデータ(下り更新用)
     */
    @Transactional
    @RequestMapping(
            value = "/download", method = RequestMethod.POST, params = { "storageid" }, headers = {
                    "Accept=application/json", "Content-Type=application/json" })
    public ResponseEntity<DownloadResponseOrdinary> syncDownload(
            final @RequestParam("storageid") String storageId,
            final @RequestBody DownloadRequest request) {

        try {

            SyncDownloadResult downloadResult =
                    synchronizer.syncDownload(storageId, request.getResources());
     ...
     *
     * @param request
     *            JSON形式の同期リクエストデータ(上り更新用)
     * @return JSON形式の同期レスポンスデータ(上り更新用)
     */
    @Transactional
    @RequestMapping(
            value = "/upload", method = RequestMethod.POST, params = { "storageid" }, headers = {
                    "Accept=application/json", "Content-Type=application/json" })
    public ResponseEntity<? extends UploadResponse> syncUpload(
            final @RequestParam("storageid") String storageId,
        ...
        try {
            ...
            // 二重送信の場合は前回処理結果を戻す
            // そうでない場合、上り更新処理を実行
            SyncUploadResult uploadResult =
                    statusService.isDuplicatedRequest(storageId, request.getDataList().hashCode())
                            ? reversionLastResult(storageId)
            ...
            case OK:
                ...
                statusService.applyUploadResult(
                        storageId, request.getDataList().hashCode(),
            ...
            case DUPLICATEDID:
                ...
                UploadResponseOnConflict responseBodyOnConflict =
                        new UploadResponseOnConflict(uploadResult);
     ...
     */
    private <T extends ResponseBody> ResponseEntity<T> createResponseEntity(
            T body,
            HttpStatus status) {
